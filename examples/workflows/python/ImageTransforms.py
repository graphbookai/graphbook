...
@g()
def _():
    data_source = g.step(HuggingfaceDataset)
    horiz_flip = g.step(HorizontalFlip)
    resized_crop = g.step(ResizedCrop)
    color_jitter = g.step(ColorJitter)
    normalize = g.step(Normalize)
    
    data_source.param("dataset_id", "kmeng/daisy-dog")
    data_source.param("split", "train")
    data_source.param("shuffle", False)
    data_source.param("log_data", True)
    data_source.param("kwargs", {})
    
    horiz_flip.param("p", 0.5)
    horiz_flip.bind(data_source, "out")
    
    resized_crop.param("size", [224, 224])
    resized_crop.param("scale", [0.08, 1.0])
    resized_crop.param("ratio", [3. / 4., 4. / 3.])
    resized_crop.bind(horiz_flip, "out")
    
    color_jitter.param("brightness", 0.5)
    color_jitter.param("contrast", 0.5)
    color_jitter.param("saturation", 0.5)
    color_jitter.param("hue", 0.5)
    color_jitter.bind(resized_crop, "out")
    
    normalize.param("mean", [0.485, 0.456, 0.406])
    normalize.param("std", [0.229, 0.224, 0.225])
    normalize.bind(color_jitter, "out")
