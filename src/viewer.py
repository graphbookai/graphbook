...
class NodeStatsViewer(Viewer):
    ...
    def handle_outputs(self, node_id: str, outputs: dict):
        ...
        if node_id not in self.start_times:
            return
        self.record_rate[node_id] = self.output_counts[node_id] / (time.time() - self.start_times[node_id])
...
class Client:
    def __init__(self, ws: WebSocketResponse, producers: List[DataViewer], options: dict = DEFAULT_CLIENT_OPTIONS):
        self.ws = ws
...
class ViewManager:
    def __init__(self, work_queue: mp.Queue, close_event: mp.Event):
        ...
        self.viewers: List[Viewer] = [self.data_viewer, self.node_stats_viewer, self.logs_viewer]
